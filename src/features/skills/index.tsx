...
import { motion, useTransform, AnimatePresence, useMotionValue, useReducedMotion, useInView, type MotionValue } from 'framer-motion';
import { skillCategories, type SkillItem, type SkillCategory } from './data/skillsData';
...
const OrbitItem: FC<OrbitItemProps> = ({ skill, color, glow, radiusX, radiusY, tilt, speed, phase, isActive, isInView }) => {
  const x = useMotionValue<number>(0);
  const y = useMotionValue<number>(0);
  const scale = useMotionValue<number>(1);
  const zIndex = useMotionValue<number>(10);
...
const OrbitRing: FC<OrbitRingProps> = ({ cat, isActive, opacity, scale }) => (
  <motion.div
    style={{ opacity, scale }}
...
const SkillsDesktop: FC<SkillsDesktopProps> = ({ scrollYProgress }) => {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const isInView = useInView(containerRef, { amount: 0.1 });
  const [scrollActiveCategory, setScrollActiveCategory] = useState<string>('core');
  ...
  const ringMotion: Record<string, { opacity: MotionValue<number>; scale: MotionValue<number> }> = {
...
const Skills: FC<SkillsProps> = ({ scrollYProgress }) => {
  if (scrollYProgress) {
